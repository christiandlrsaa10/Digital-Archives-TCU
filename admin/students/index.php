							...
							<td align="center">
								 <button type="button" class="btn btn-flat btn-default btn-sm dropdown-toggle dropdown-icon" data-toggle="dropdown">
				                  ...
				                  <div class="dropdown-menu" role="menu">
									...
									<?php if($row['status'] == 1): ?>
										<a class="dropdown-item deact_user" href="javascript:void(0)" data-id="<?= $row['id'] ?>"  data-name="<?= $row['name'] ?>"><span class="fa fa-close text-primary text-secondary"></span> Deactivate</a>
									<?php else: ?>
...
<script>
	$(document).ready(function(){
		...
		$('.table td,.table th').addClass('py-1 px-2 align-middle')
		$('.deact_user').click(function(){
			_conf("Are you sure to deactivate <b>"+$(this).attr('data-name')+"<b/>?","deact_user",[$(this).attr('data-id')])
		})
	...
	function deact_user($id){
		start_loader();
		$.ajax({
			url:_base_url_+"classes/Users.php?f=deact_user",
			method:"POST",
			data:{id: $id},
			dataType:"json",
			error:err=>{
